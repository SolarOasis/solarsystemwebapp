...
import { Sun, Battery, TrendingUp, FileText, AlertCircle, Trash2, PlusCircle, Download, XCircle, Wand2, Info, Upload, ChevronsDown, ChevronsUp, Leaf, Car, TreePine, Printer, Copy } from 'lucide-react';
...
const CalculatorPage: React.FC = () => {
    ...
    for (let year = 1; year <= 25; year++) {
      ...
      for (const month of months) {
        ...
        if (authority === 'DEWA') {
            ...
            monthNetMeterCredit = calculateBillAmount(gridExport, escalatedTiers);
            
            const selfConsumedSolar = monthlyProduction - gridExport;
            ...
            const selfConsumedSolar = Math.min(monthlyProduction, daytimeKwh);
            gridImport = monthlyConsumption - selfConsumedSolar;
            newBillOnTiers = calculateBillAmount(gridImport, escalatedTiers);
            monthFuelSavings = selfConsumedSolar * escalatedFuelSurcharge;
        }
  ...
  const copyReport = () => {
    ...
    const gridVsSolarSplit = `Covered by Solar: ${Math.round(reportData.annualProduction).toLocaleString()} kWh\nRemaining from Grid: ${Math.max(0, Math.round(reportData.annualConsumption - reportData.annualProduction)).toLocaleString()} kWh`;
            ...
            <div className="space-y-6">
              <Card title="Recommended System">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="p-4 rounded-lg bg-brand-secondary text-brand-primary"><p>Roof Space Required</p><p className="text-2xl font-bold">~{systemMetrics.spaceRequired.toFixed(1)} m²</p></div>
                  </div>
                   {productionBoost > 0.1 && (
