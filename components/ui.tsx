...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(({ label, id, helperText, error, className, ...props }, ref) => {
    return (
        <div className="w-full">
            ...
            <input
                id={id}
                ref={ref}
                className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-brand-primary focus:border-brand-primary sm:text-sm ${error ? 'border-red-500' : 'border-gray-300'} ${className || ''}`}
                {...props}
            />
            {helperText && <p className={`mt-1 text-xs ${error ? 'text-red-600' : 'text-gray-500'}`}>{helperText}</p>}
        </div>
    );
});
